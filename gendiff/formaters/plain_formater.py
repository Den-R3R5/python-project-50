...
def plain(tree, depth=1, directory=""):
    ...
    for node in tree:
        ...
        if node_type == "attached":
            ...
            result.append(
                f"Property '{directory}{key}' was updated. From {_format(node['value_old'], depth)} to {_format(node['value_new'], depth)}"
            )
        elif node_type == "deleted":
            result.append(f"Property '{directory}{key}' was removed")
        elif node_type == "added":
            result.append(
                f"Property '{directory}{key}' was added with value: {_format(node['value'], depth)}"
            )
    return "\n".join(result)
